...
class WA_Admin {
	...
	public function includes() {
		include_once( 'class-wa-options.php' );
		include_once( 'class-wa-metabox.php' );
		include_once( 'wa-admin-functions.php' );
	}
	...
	public function admin_init_hooks() {
		...
		add_filter( 'manage_gallery_posts_columns', array( $this, 'admin_columns_head_gallery_thumb' ), 10 );
		add_action( 'manage_gallery_posts_custom_column', array( $this, 'admin_columns_content_gallery_thumb' ), 10, 2 );
	}
	...
	public function check_page() {
		...
		// delete_option( '_wolf_albums_no_needs_page', true );
		// delete_option( '_wolf_albums_page_id' );

		if ( get_option( '_wolf_albums_no_needs_page' ) )
			return;
	...
	/**
	 * Hide the editor if we're on the admin albums page
	 */
	 ...
	 * @param int $post_id
	 */
	public function admin_columns_content_gallery_thumb( $column_name, $post_id ) {
	...
	public function plugin_update() {
		...
		$plugin_slug = WA_SLUG;
		$plugin_path = WA_PATH;
		$remote_path = WA_UPDATE_URL . '/' . $plugin_slug;
		$plugin_data = get_plugin_data( WA_DIR . '/' . WA_SLUG . '.php' );
		$current_version = $plugin_data['Version'];
		include_once( 'class-wa-update.php');
		new WA_Update( $current_version, $remote_path, $plugin_path );
	}
}
