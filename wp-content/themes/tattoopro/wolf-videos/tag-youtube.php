...
$current_tax = get_query_var( 'video_tag' );

$args = array(
	'post_type' => 'video',
	'posts_per_page' => $posts_per_page,
	'video_tag' => $current_tax,
	'meta_query' => array(
		array(
			'key' => '_thumbnail_id',
			'compare' => '!=',
	...
	<div class="video-inner inner">
		<?php
			if ( $loop->have_posts() ) :
				...
				echo '<section class="clearfix video-category ' . $current_tax . '">';
			...
			<?php
				...
				<article <?php post_class( 'clearfix' ); ?>   id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>">
					<span class="video-thumbnail">
						<a class="entry-link" href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_post_thumbnail( 'classic-video-thumb' ); ?></a>
					</span>
					...
					<span class="video-date">
						<?php printf( __( '%s ago', 'tattoopro' ), human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) ); ?>
					</span>
				</article>
