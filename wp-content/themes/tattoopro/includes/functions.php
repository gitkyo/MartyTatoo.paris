...
if ( ! function_exists( 'wolf_more_text' ) ) {
	...
	function wolf_more_text() {
		...
		$format    = null;
		$text      = __( 'Continue reading &rarr;', 'tattoopro' );
		$format    = get_post_format();
		$post_type = get_post_type();

		if ( 'video' == $format || 'video' == $post_type ) {

			$text = __( 'More about this video &rarr;', 'tattoopro' );

		} elseif ( 'gallery' == $format || 'image' == $format || 'gallery' == $post_type ) {

			$text = __( 'View more &rarr;', 'tattoopro' );

		} elseif ( 'audio' == $format || 'release' == $post_type || 'show' == $post_type ) {

			$text = __( 'More info &rarr;', 'tattoopro' );
...
if ( ! function_exists( 'wolf_display_author_socials' ) ) {
	...
	function wolf_display_author_socials() {
		...
		if ( $website ) {
			echo '<a target="_blank" title="' . sprintf( __( 'Visit %s website', 'tattoopro' ), $name ) . '" href="'. $website .'" class="author-link">' . __( 'website', 'tattoopro' ) . '</a>';
		}
		...
		foreach ( $team_member_socials as $social ) {
			...
			if ( get_the_author_meta( $social ) ) {
				echo '<a target="_blank" title="' . sprintf( __( 'Find %1$s on %2$s', 'tattoopro' ), $name, $social_name ) . '" href="'. esc_url( $url ) .'" class="author-link">' . $social_name . '</a>';
			}
		}
	}
}
...
if ( ! function_exists( 'wolf_posts_link_next_title' ) ) {
	...
	function wolf_posts_link_next_title() {
		return 'title="' . __( 'Older', 'tattoopro' ) . '"';
	}
...
if ( ! function_exists( 'wolf_posts_link_prev_title' ) ) {
	...
	function wolf_posts_link_prev_title() {
		return 'title="' . __( 'Newer', 'tattoopro' ) . '"';
	}
		...
		if ( have_posts() ) {
			...
			if ( ! is_404() && ! wolf_is_woocommerce() ) {
				...
				if ( is_category() ) {
					...
					$title = sprintf( __( 'Search Results for: %s', 'tattoopro' ), get_search_query() );
...
if ( ! function_exists( 'all_rev_sliders_in_array' ) ) {
	...
	function all_rev_sliders_in_array() {
		...
		if ( $arrA && $arrT ) {
			...
			$result = array( '' => __( 'No slider yet', 'tattoopro' ) );
		}
			return $result;
		}
	}
