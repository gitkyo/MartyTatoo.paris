		...
		$message = sprintf(
			wp_kses(
				__( 'Woulf you like to try our <a href="%1$s" target="_blank"><strong>Wolf Page Builder</strong></a> instead of Visual Composer? It is simple, light, supported, automatically updated and it\'s totally <strong>FREE</strong>!
					...
					&nbsp;
					<a href="%3$s" class="button button-primary">Try Now</a>', 'tattoopro' ),
				array(
					'a' => array(
						'href' => array(),
						'class' => array(),
...
if ( ! function_exists( 'wolf_add_media_manager_options' ) ) {
	...
	function wolf_add_media_manager_options() {
		...
		<script type="text/html" id="tmpl-custom-gallery-setting">
			...
			<label class="setting">
				<span><?php _e( 'Layout', 'tattoopro' ); ?></span>
				<select data-setting="layout">
					<option value="simple"><?php _e( 'Simple', 'tattoopro' ); ?></option>
					<option value="mosaic"><?php _e( 'Mosaic', 'tattoopro' ); ?></option>
					<?php if ( 'gallery' != get_post_type() ) : ?>
						<option value="carousel_simple"><?php _e( 'Simple Carousel', 'tattoopro' ); ?></option>
					<?php endif; ?>
					<?php if ( 'gallery' == get_post_type() ) : ?>
					<option value="masonry"><?php _e( 'Masonry', 'tattoopro' ); ?></option>
					<?php endif; ?>
				</select>
			</label>
			<label class="setting">
				<span><?php _e( 'Size', 'tattoopro' ); ?></span>
				<select data-setting="size">
					<option value="classic-thumb"><?php _e( 'Standard', 'tattoopro' ); ?></option>
					<option value="2x2"><?php _e( 'Square', 'tattoopro' ); ?></option>
					<option value="portrait"><?php _e( 'Portrait', 'tattoopro' ); ?></option>
				</select>
			</label>
			<label class="setting">
				<span><?php _e( 'Padding', 'tattoopro' ); ?></span>
				<select data-setting="padding">
					<option value="yes"><?php _e( 'Yes', 'tattoopro' ); ?></option>
					<option value="no"><?php _e( 'No', 'tattoopro' ); ?></option>
				</select>
			</label>
			<label class="setting">
				<span><?php _e( 'Hover effect', 'tattoopro' ); ?></span>
				<select data-setting="hover_effect">
					<option value="default"><?php _e( 'Default', 'tattoopro' ); ?></option>
					<option value="greyscale"><?php _e( 'Black and white to colored', 'tattoopro' ); ?></option>
					<option value="to-greyscale"><?php _e( 'Colored to Black and white', 'tattoopro' ); ?></option>
					<option value="scale-greyscale"><?php _e( 'Scale + Black and white to colored', 'tattoopro' ); ?></option>
					<option value="scale-to-greyscale"><?php _e( 'Scale + Colored to Black and white', 'tattoopro' ); ?></option>
					<option value="none"><?php _e( 'None', 'tattoopro' ); ?></option>
				</select>
				<small><?php _e( 'Note that not all browser support the black and white effect', 'tattoopro' ); ?></small>
			</label>
		</script>
