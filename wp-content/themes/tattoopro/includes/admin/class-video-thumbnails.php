...
'Error retrieving video information from the URL <a href="%1$s">%1$s</a>.<a href="%2$s">Libcurl error</a> %3$s: <code>%4$s</code>. If opening that URL in your web browser returns anything else than an error page, the problem may be related to your web server and might be something your host administrator can solve.',
		...
		public function render_metabox() {
			...
			if ( get_post_status() == 'publish' || get_post_status() == 'private' ) {
				...
				if ( isset( $video_thumbnail ) && $video_thumbnail != '' ) {
					...
					echo '<p id="video-thumbnail-action">';
					echo '<a href="#" id="video-thumbnail-reset" onClick="video_thumbnail_reset(\'' . $post_id . '\' );return false;">' . __( 'Reset', 'tattoopro' ) . '</a>';
					echo '<a href="#" id="video-thumbnail-delete" style="display:inline-block; border-left:1px dotted #ccc; margin-left:5px;padding-left:6px" onClick="video_thumbnail_delete(\'' . $post_id . '\' );return false;">' . __( 'Delete', 'tattoopro' ) . '</a>';
					echo '</p>';
		...
		public function video_thumbnail_script() {
			...
			function video_thumbnail_reset( id ) {
				...
				var removeFeaturedImg = '<p id="video-custom-remove" class="hide-if-no-js"><a href="#" id="remove-post-thumbnail" onclick="wolf_thumbnail_delete( ' + id + ' );return false;"><?php _e( 'Remove featured image', 'tattoopro' ); ?></a></p>';
				...
				document.getElementById( 'video-thumbnail-preview' ).innerHTML= '<?php _e( 'Working...', 'tattoopro' ); ?>... <img src="<?php echo admin_url( 'images/loading.gif' ); ?>"/>';
			...
			function wolf_thumbnail_delete( id ) {
				...
				jQuery.post( ajaxurl, data, function( response ) {
					jQuery( '#postimagediv .inside #set-post-thumbnail' ).empty().html( '<?php _e( 'Set featured image', 'tattoopro' ); ?>' );
					jQuery( '#remove-post-thumbnail' ).remove();
				} );
			}
			</script>
		...
		public function get_text( $string ) {
			...
			// Translatable string
			$text = array(
				'no_video' => __( 'No video URL in the post.', 'tattoopro' ),
				'has_video' => __( 'A video has been attached to this post. Click on "Generate" to create a thumbnail from this video', 'tattoopro' ),
				'not_published' => __( 'You will be able to generate a video thumbnail and use it as featured image for this post when it is published.', 'tattoopro' ),
				'generate' => __( 'Generate', 'tattoopro' ),
				'reset' => __( 'Reset', 'tattoopro' ),
				'delete' => __( 'Delete', 'tattoopro' ),
			);
