	...
	*/
	class Wolf_Admin_Welcome_Page {
		...
		public function admin_menus() {
			...
			// $welcome_page_name  = sprintf( __( 'About %s', 'tattoopro' ), $theme_name );
			// $welcome_page_title = sprintf( __( 'Welcome to %s', 'tattoopro' ), $theme_name );
			...
			add_submenu_page( 'wolf-theme-options', __( 'About', 'tattoopro' ), __( 'About', 'tattoopro' ), 'manage_options', 'wolf-about', array( $this, 'about_screen' ) );
		}
		...
		private function intro() {
			...
			?>
			<h1><?php printf( __( 'Welcome to %s %s', 'tattoopro' ), $theme_name, $theme_version ); ?></h1>

			<div class="about-text wolf-about-text">
				<?php
					if ( isset( $_GET['wolf-theme-updated'] ) )
						$message = __( 'Thank you for updating to the latest version!', 'tattoopro' );
					else
						$message = sprintf( __( 'Thanks for installing %s!', 'tattoopro' ), $theme_name );

					if ( isset( $_GET['wolf-theme-activated'] ) ) {
						printf( __( '%s We hope you will enjoy using it.', 'tattoopro' ), $message );
					} elseif ( isset( $_GET['wolf-theme-updated'] ) ) {
						printf( __( '%s <br> %s is now more stable and secure than ever.<br>We hope you enjoy using it.', 'tattoopro' ), $message, $theme_name );
					} else {
						printf( __( '%s We hope you enjoy using it.', 'tattoopro' ), $message );
					}
				?>
			</div>
		...
		public function features() {
			...
			<p class="wolf-about-actions">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=wolf-theme-options' ) ); ?>" class="button button-primary"><?php _e( 'Settings', 'tattoopro' ); ?></a>
				<a
		...
		public function new_features() {
			...
			//if ( isset( $_GET['wolf-theme-updated'] ) ) {
				...
				if ( wolf_get_last_change_from_changelog() ) {
					?>
					<h2 style="text-align:left;"><?php printf( __( 'What\'s new in %s', 'tattoopro' ), WOLF_THEME_VERSION ); ?></h2>
					<?php
