...
if ( ! function_exists( 'wolf_image_attachment_add_custom_fields' ) ) {
	...
	function wolf_image_attachment_add_custom_fields( $form_fields, $post ) {
		...
		$form_fields['wolf_custom_size'] = array(
			'label' => __( 'Custom Size', 'tattoopro' ),
			'input' => 'html',
			'helps' => __( 'This settings will be applied for the "mosaic gallery" only.', 'tattoopro' ),
			'application' => 'image',
		...
		$options = array(
			'small-square' => __( 'Small Square', 'tattoopro' ),
			'big-square' => __( 'Big Square', 'tattoopro' ),
			'portrait' => __( 'portrtait', 'tattoopro' ),
			'landscape' => __( 'landscape', 'tattoopro' ),
