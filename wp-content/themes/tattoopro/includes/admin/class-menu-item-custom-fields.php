...
class Wolf_Menu_Item_Custom_Fields {
	...
	public static function _fields( $item, $depth, $args = array(), $id = 0 ) {
		...
		?>
			...
			<p class="field-_sub-menu-skin description description-wide">
				<label for="edit-_sub-menu-skin-<?php echo esc_attr( $item_id ) ?>"><?php _e( 'Sub menu skin (only for first level item)', 'tattoopro' ) ?></label>
					<br>
					<select name="_sub-menu-skin[<?php echo esc_attr( $item_id ); ?>]">
						<option value="sub-menu-dark" <?php selected( get_post_meta( $item_id, '_sub-menu-skin', true ), 'sub-menu-dark' ); ?>><?php _e( 'dark', 'tattoopro' ); ?></option>
						<option value="sub-menu-light" <?php selected( get_post_meta( $item_id, '_sub-menu-skin', true ), 'sub-menu-light' ); ?>><?php _e( 'light', 'tattoopro' ); ?></option>
					</select>
			</p>
			<p class="field-_mega-menu description description-wide">
				<label for="edit-_mega-menu-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php _e( 'Mega Menu (only available for first level items)', 'tattoopro' ) ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-not-linked description description-wide">
				<label for="edit-_menu-item-not-linked-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php _e( 'Mega Menu 2nd level or dropdown item', 'tattoopro' ) ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-hidden description description-wide">
				<label for="edit-_menu-item-hidden-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php _e( 'Hide item on mega menu (for mega menu 2nd level only)', 'tattoopro' ) ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-button-style description description-wide">
				<label for="edit-_menu-item-button-style-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php _e( 'Button Style (only available for first level items)', 'tattoopro' ) ?>
				</label>
			</p>
			...
			<p class="field-_menu-item-scroll description description-wide">
				<label for="edit-_menu-item-scroll-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php _e( 'Scroll to an anchor', 'tattoopro' ) ?>
				</label>
			</p>
			<p class="field-_menu-item-external description description-wide">
				<label for="edit-_menu-item-external-<?php echo esc_attr( $item_id ) ?>">
					...
					<?php _e( 'External link?', 'tattoopro' ) ?>
				</label>
			</p>
			<p class="field-background description description-wide">
				...
				<label for="edit-_menu-item-background-<?php echo esc_attr( $item_id ) ?>">
					<?php _e( 'Background Image (only for 1rst level mega menu)', 'tattoopro' ) ?>
			...
			<p class="field-_menu-item-background-repeat description description-wide">
				<label for="edit-_menu-item-background-repeat-<?php echo esc_attr( $item_id ) ?>"><?php _e( 'Background repeat', 'tattoopro' ) ?></label><br>
					<select name="_menu-item-background-repeat[<?php echo esc_attr( $item_id ); ?>]">
						<option value="no-repeat" <?php selected( get_post_meta( $item_id, '_menu-item-background-repeat', true ), 'no-repeat' ); ?>><?php _e( 'no repeat', 'tattoopro' ); ?></option>
						<option value="repeat" <?php selected( get_post_meta( $item_id, '_menu-item-background-repeat', true ), 'repeat' ); ?>><?php _e( 'repeat', 'tattoopro' ); ?></option>
					</select>
			</p>
			<p class="field-custom description description-wide wolf-searchable-container">
				<label for="edit-_menu-item-icon-<?php echo esc_attr( $item_id ) ?>"><?php _e( 'Icon', 'tattoopro' ) ?></label><br />
					...
					echo '<option value="">' . __( 'None', 'tattoopro' ) . '</option>';
			...
			<p class="field-_menu-item-icon-position description description-wide">
				<label for="edit-_menu-item-icon-position-<?php echo esc_attr( $item_id ) ?>"><?php _e( 'Icon position', 'tattoopro' ) ?></label>
					<br>
					<select name="_menu-item-icon-position[<?php echo esc_attr( $item_id ); ?>]">
						<option value="before" <?php selected( get_post_meta( $item_id, '_menu-item-icon-position', true ), 'before' ); ?>><?php _e( 'before', 'tattoopro' ); ?></option>
						<option value="after" <?php selected( get_post_meta( $item_id, '_menu-item-icon-position', true ), 'after' ); ?>><?php _e( 'after', 'tattoopro' ); ?></option>
					</select>
			</p>
		<?php
	}
	...
	public static function _columns( $columns ) {
		//$columns['_menu-item-icon'] = __( 'Icon', 'tattoopro' );
		//$columns['_mega-menu'] = __( 'Mega Menu', 'tattoopro' );
		//$columns['_menu-item-not-linked'] = __( 'Sub Menu title type', 'tattoopro' );
		//$columns['_menu-item-hidden'] = __( 'Sub Menu title type', 'tattoopro' );
