...
if ( ! function_exists( 'wolf_post_entry_meta' ) ) {
	...
	function wolf_post_entry_meta( $echo = true ) {
		...
		if ( is_sticky() && is_home() && ! is_paged() )
			$output .= '<span class="featured-post">' . __( 'Featured', 'tattoopro' ) . '</span>';
		...
		if ( 'post' == get_post_type() && is_multi_author() ) {
			$output .= sprintf(
				...
				esc_attr( sprintf( __( 'View all posts by %s', 'tattoopro' ), get_the_author() ) ),
		...
		if ( 'work' == get_post_type() ) {
			...
			$categories_list = get_the_category_list( __( ', ', 'tattoopro' ) );
		}
		...
		// Translators: used between list items, there is a space after the comma.
		$tag_list = get_the_tag_list( '', __( ', ', 'tattoopro' ) );
		if ( $tag_list ) {
...
if ( ! function_exists( 'wolf_entry_date' ) ) {
	...
	function wolf_entry_date( $echo = true, $link = true ) {
		...
		if ( 'human_diff' == wolf_get_theme_option( 'date_format' ) ) {
			$display_time = sprintf( __( '%s ago', 'tattoopro' ), human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) );
			$modified_display_time = sprintf( __( '%s ago', 'tattoopro' ), human_time_diff( get_the_modified_time( 'U' ), current_time( 'timestamp' ) ) );
		}
	...
	function wolf_paging_nav( $loop = null ) {
		...
		<nav class="navigation paging-navigation" role="navigation">
			<div class="nav-links clearfix">
				...
				<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'tattoopro' ), $max ); ?></div>
				<?php endif; ?>

				<?php if ( get_previous_posts_link( '', $max ) ) : ?>
				<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'tattoopro' ), $max ); ?></div>
				<?php endif; ?>
	...
	function wolf_post_nav() {
		...
		<nav class="nav-single clearfix">
			...
			<div class="nav-previous"<?php
				...
				<?php endif; ?>>
				<?php previous_post_link( '%link', '<span class="nav-label">' . __( 'Previous post', 'tattoopro' ) . '</span><span class="meta-nav"></span> %title' ); ?>
			</div>
			<div class="nav-next"<?php
				...
				<?php endif; ?>>
				<?php next_post_link( '%link', '<span class="nav-label">' . __( 'Next post', 'tattoopro' ) . '</span> %title <span class="meta-nav"></span>' ); ?>
			</div>
		</nav><!-- .nav-single -->
		<?php
	}
	...
	function wolf_icon_meta() {
		...
		?>
			<?php if ( comments_open() && ( wolf_get_theme_option( $post_type . '_comments' ) || 'post' == $post_type ) ) : ?>
				<span class="item-icon" title="<?php printf( __( '%d comments', 'tattoopro' ), $comments ); ?>">
			...
			<?php if ( wolf_get_theme_option( $post_type . '_likes' ) ) : ?>
				<span class="item-icon item-like" title="<?php _e( 'Like this', 'tattoopro' ); ?>">
			...
			<?php if ( wolf_get_theme_option( $post_type . '_views' ) && ! in_array( $format, $no_views ) ) : ?>
				<span class="item-icon" title="<?php printf( __( '%d views', 'tattoopro' ), $views ); ?>">
...
if ( ! function_exists( 'wolf_theme_work_meta' ) ) {
	...
	function wolf_theme_work_meta() {
		...
		$skills = get_the_term_list( $post_id, 'work_type', '', __( ', ', 'tattoopro' ), '' );
		$display_date = wolf_get_theme_option( 'work_date' );
		?>
		<?php if ( $display_date ) : ?>
			<span class="work-meta work-date">
				<i class="fa line-icon-calendar"></i> <strong><?php _e( 'Date', 'tattoopro' ); ?></strong> : <?php wolf_entry_date(); ?>
			</span>
		<?php endif; ?>

		<?php if ( $skills ) : ?>
			<span class="work-meta work-categories">
				<i class="fa line-icon-tag"></i> <strong><?php _e( 'Category', 'tattoopro' ); ?></strong> : <?php echo sanitize_text_field( $skills ); ?>
			</span>
		<?php endif; ?>

		<?php if ( $client ) : ?>
			<span class="work-meta work-client">
				<i class="fa line-icon-user"></i> <strong><?php _e( 'Client', 'tattoopro' ); ?></strong> : <?php echo sanitize_text_field( $client ); ?>
			</span>
		<?php endif; ?>
		...
		?>
			<span class="work-meta work-link">
				<i class="fa fa-external-link"></i> <strong><?php _e( 'Link', 'tattoopro' ); ?></strong> : <a href="<?php echo esc_url( $link ); ?>"><?php echo sanitize_text_field( $link_text ); ?></a>
			</span>
		<?php endif; ?>
		<?php wolf_icon_meta(); ?>
		<?php
