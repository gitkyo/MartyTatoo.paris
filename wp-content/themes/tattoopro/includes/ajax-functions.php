	...
	function wolf_mailchimp_ajax() {
		...
		if ( isset( $_POST['email'] ) && isset( $_POST['list_id'] ) ) {
			...
			if ( is_email( $email ) ) {
				...
				_e( 'Subscription successful', 'tattoopro' );

			} else {
				_e( 'Please insert a valid email', 'tattoopro' );
			}
			exit;
		}
	}
...
if ( class_exists( 'Woocommerce' ) ) {
	...
	if ( ! function_exists( 'wolf_woocommerce_add_to_cart_fragment_item_count_with_text' ) ) {
		...
		function wolf_woocommerce_add_to_cart_fragment_item_count_with_text( $fragments ) {
			...
			ob_start();
			?>
			<span class="panel-product-count"><?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'tattoopro'), $woocommerce->cart->cart_contents_count ); ?></span>
			<?php
	...
	if ( ! function_exists( 'wolf_woocommerce_add_to_cart_fragment_total' ) ) {
		...
		function wolf_woocommerce_add_to_cart_fragment_total( $fragments ) {
			...
			ob_start();
			?>
			<span class="panel-total"><?php _e( 'Total:', 'tattoopro' ); ?> <?php echo sanitize_text_field( $woocommerce->cart->get_cart_total() ); ?></span>
			<?php
