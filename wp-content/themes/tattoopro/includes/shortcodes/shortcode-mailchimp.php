...
if ( ! function_exists( 'wolf_mailchimp' ) ) {
	...
	function wolf_mailchimp( $list, $size = 'normal', $label = '', $submit = 'Subscribe', $button_style = '', $alignment = 'center', $animation = '', $animation_delay = '', $inline_style = '', $class = '' ) {
		...
		$output .= '<div class="wolf-mailchimp-email-container"><input placeholder="' . __( 'your email', 'tattoopro' ) . '"  type="text" name="wolf-mailchimp-email" class="wolf-mailchimp-email"></div>';
		...
		if ( wolf_get_theme_option( 'mailchimp_api_key' ) && ! empty( $list ) ) {
			...
			if ( ! wolf_get_theme_option( 'mailchimp_api_key' ) )
				$output .= __( 'You must set your mailchimp API key in the theme options', 'tattoopro' ) . '<br>';

			if ( ! $list )
				$output .= __( 'You must set a list ID!', 'tattoopro' );
