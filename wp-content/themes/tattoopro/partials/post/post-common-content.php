...
<article <?php post_class( 'clearfix' ); ?>   id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>" data-bg="<?php echo esc_url( $bg ); ?>">
	...
	<div class="entry-content">
		<div class="entry-inner">
			<?php if ( $format != 'link' && $format != 'quote' ) : ?>
				<?php if ( $format != 'aside' && $format != 'status' && $format != 'link' && $format != 'quote' ) : ?>
					<h2 class="entry-title">
						<a class="entry-link" href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'tattoopro' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a>
					</h2>
				<?php endif; ?>
				<div class="entry-meta">
					<?php wolf_post_entry_meta(); ?>
					<?php edit_post_link( __( 'Edit', 'tattoopro' ), '<span class="edit-link">', '</span>' ); ?>
				</div>
			...
			<?php endif; ?>
			<?php wp_link_pages( array( 'before' => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'tattoopro' ) . '</span>', 'after' => '</div>', 'link_before' => '<span>', 'link_after' => '</span>' ) ); ?>
