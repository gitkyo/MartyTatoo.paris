	...
	class Wolf_Theme_Admin_Default_Pages {
		...
		public function display_notice() {
			...
			if ( ! get_option( '_wolf_' . wolf_get_theme_slug() . '_default_pages' ) && array() != $this->default_posts  ) {
				...
				$default_pages_install_message = sprintf( __( 'You can install default pages to help you to get started with <strong>%s</strong> theme.', 'tattoopro' ), WOLF_THEME_NAME );

				if ( function_exists( 'wolf_theme_register_required_plugins' ) ) {

					$default_pages_install_message .= '<br>'  . __( '<strong>It is recommended to install and activate the plugins you need from the recommended plugins list first</strong>, in order to install the default pages accordingly.', 'tattoopro' ) .'<br>';
				}

				$default_pages_install_message .= '<br>' . sprintf(
					__( '<a href="%1$s" class="button-primary">Install default pages</a> <a href="%2$s" class="button">Dismiss this message</a>', 'tattoopro' ),
