		...
		public function background_setting( $option, $section_id, $wp_customize, $section = true ) {
			...
			if ( $is_no_bg ) {
				...
				$wp_customize->add_control(
					...
					array(
						'type' => 'checkbox',
						'label' => __( 'No Background', 'tattoopro' ),
						'section' => $section_id,
						'priority' => 1,
					)
				);
			...
			$wolf_wp_customize_color_control = new WP_Customize_Color_Control(
				...
				$background_id . '_color',
					array(
						'label' => __( 'Background Color', 'tattoopro' ),
						'section' => $section_id,
			...
			if ( $opacity ) :
				...
				$wp_customize->add_control(
					$background_id . '_opacity',
					array(
						'label' => __( 'Background color opacity (in percent)', 'tattoopro' ),
						'section' => $section_id,
						'type' => 'text',
						'priority' => 3,
					)
			...
			if ( $is_bg_img ) :
				...
				$wolf_wp_customize_image_control = new WP_Customize_Image_Control(
					...
					$background_id . '_img',
						array(
							'label' => __( 'Background Image', 'tattoopro' ),
							'section' => $section_id,
				...
				$wp_customize->add_control(
					...
					array(
						'type' => 'select',
						'label' => __( 'Background Repeat', 'tattoopro' ),
						'section' => $section_id,
						'choices' => array(
							'repeat' => 'repeat',
							'no-repeat' => 'no-repeat',
				...
				$wp_customize->add_control(
					...
					array(
						'type' => 'select',
						'label' => __( 'Background Position', 'tattoopro' ),
						'section' => $section_id,
						'choices' => array(
				...
				$wp_customize->add_control(
					...
					array(
						'type' => 'select',
						'label' => __( 'Background Attachment', 'tattoopro' ),
						'section' => $section_id,
						'choices' => array(
							'scroll' => 'scroll',
							'fixed' => 'fixed',
				...
				$wp_customize->add_control(
					...
					array(
						'type' => 'select',
						'label' => __( 'Background Size', 'tattoopro' ),
						'section' => $section_id,
						'choices' => array(
	...
	function wolf_customizer_add_custom_sections( $wp_customize ) {
		...
		class Wolf_Customize_Textarea_Control extends WP_Customize_Control {
			...
			public function __construct( $manager, $id, $args = array() ) {
				$this->statuses = array( '' => __( 'Default', 'tattoopro' ) );
		...
		class Wolf_Customize_Skins_Control extends WP_Customize_Control {
			...
			public function render_content() {
				$reset_options_confirm = __( 'Are you sure to want to reset all styles to default ?', 'tattoopro' );
				?>
				<label>
					<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
					...
					foreach ( $this->choices as $value => $label ) : ?>
						...
						<?php endforeach; ?>
					<a  style="margin-bottom:5px;" class="button wolf-preset-button" rel="reset" href="<?php echo esc_url( admin_url( 'admin.php?page=wolf-customizer-presets&amp;skin=reset' ) ); ?>"><?php _e( 'reset all', 'tattoopro' ); ?></a>
					<?php endif; ?>
				</label>
				<?php
			}
