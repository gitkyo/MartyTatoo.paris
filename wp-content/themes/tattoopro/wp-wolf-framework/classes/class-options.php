	...
	class Wolf_Theme_Admin_Options {
		...
		public function save() {
			...
			if ( isset( $_GET['message'] ) && 'save' == $_GET['message'] && isset( $_GET['page'] ) && $_GET['page'] == 'wolf-theme-options' ) {
				wolf_admin_notice( __( 'Your settings have been saved.', 'tattoopro' ), 'updated' );
			}

			if ( isset( $_GET['message'] ) && 'export' == $_GET['message'] && isset( $_GET['page'] ) && $_GET['page'] == 'wolf-theme-options' ) {
				wolf_admin_notice( __( 'Your download should start in a few seconds.', 'tattoopro' ), 'updated' );
			...
			if ( isset( $_GET['page'] ) && $_GET['page'] == 'wolf-theme-options' ) {
					...
					foreach ( $this->options as $value ) {
						...
						elseif ( 'email' == $type ) {
							...
							if ( ! empty( $data[ $value_key ] ) && ! is_email( $data[ $value_key ] ) ) {
								...
								$errors[] = '<strong>' . $data[ $value_key ] . '</strong> '.__( 'is not a valid email', 'tattoopro' ).'.';
				...
				if ( ( isset( $_POST['action'] ) ) && ( $_POST['action'] == 'wolf-reset-all-options' ) ) {
					wolf_admin_notice( __( 'Your settings have been reset.', 'tattoopro' ), 'updated' );
				}
		...
		public function import_options() {
			if ( isset( $_POST['wolf-options-import-file-submit'] ) ) {
				...
				if ( ! empty( $_FILES['wolf-options-import-file']['name'] ) ) {
					...
					if ( 'zip' != $ext && 'txt' != $ext ) {
						$message = __( 'Only .txt file or zip file containing a text file are allowed', 'tattoopro' );
						...
						if ( 'zip' == $ext ) {
							...
							if ( $zip->open( $file['tmp_name'] ) === TRUE ) {
								...
								if ( $txt_file ) {
									...
									$message = __( 'It seems that your archive is empty or does not contain a txt file', 'tattoopro' );
									wolf_admin_notice( $message, 'error' );
								}

							} else {
								$message = __( 'We could not import the theme options', 'tattoopro' );
								wolf_admin_notice( $message, 'error' );
							}
						...
						if ( '' != $file_content ) {
							...
							if ( $data && is_array( $data ) ) {
								...
								wolf_admin_notice( __( 'New options imported successfully', 'tattoopro' ), 'updated' );
							} else {
								wolf_admin_notice( __( 'The file doesn\'t seem to contain any options.', 'tattoopro' ), 'updated' );
							}

						} else {
							$message = __( 'Looks like your text file is empty', 'tattoopro' );
							wolf_admin_notice( $message, 'error' );
						}
					...
					$message = __( 'Please Select a file to upload', 'tattoopro' );
					wolf_admin_notice( $message, 'error' );
				}
				return false;
			}
		...
		public function render() {
			...
			if ( is_child_theme() && WOLF_UPDATE_NOTICE )
				$theme_version = sprintf( __( 'v.%1$s (Parent Theme v.%2$s)', 'tattoopro' ), wp_get_theme()->Version, WOLF_THEME_VERSION ) ;
			...
			<h2 class="nav-tab-wrapper">
				...
				<div class="tabs" id="tabs1">
					...
					if ( class_exists( 'ZipArchive' ) && WOLF_ENABLE_OPTIONS_EXPORTER ) : ?>
						<a href="#import" class="nav-tab"><?php _e( 'Export/Import', 'tattoopro' ); ?></a>
					<?php endif; ?>
				</div>
			</h2>
	...
	<?php foreach ( $this->options as $value ) {
		...
		?>
			<div class="wolf-options-actions">
				<span class="submit">
					<input name="wolf-theme-options-save" type="submit" class="wolf-theme-options-save button-primary menu-save" value="<?php _e( 'Save changes', 'tattoopro' ); ?>">
		...
		<?php if ( class_exists( 'ZipArchive' ) && WOLF_ENABLE_OPTIONS_EXPORTER ) : ?>
				<div id="import" class="wolf-options-panel">
						<p><?php _e( 'Here you can export or import your theme options in zip file format', 'tattoopro' ); ?></p>
						<p>
							<input type="file" name="wolf-options-import-file">
							<input id="wolf-import-options" type="submit" name="wolf-options-import-file-submit" class="button" value="<?php _e( 'Import Options', 'tattoopro' ); ?>">
						</p>
						<p>
							<input id="wolf-export-options" type="submit" name="wolf-options-export-file-submit" class="button" value="<?php _e( 'Export Options', 'tattoopro' ); ?>">
						</p>
				</div>
		 <?php endif; ?>
		...
		<?php
		$reset_options_confirm = __( 'Are you sure to want to reset all options ?', 'tattoopro' );
		?>
		<div id="wolf-options-footer">
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=wolf-theme-options' ) ); ?>">
				<p id="reset">
					<input name="wolf-reset-all-options" type="submit" value="<?php _e( 'Reset all options', 'tattoopro' ); ?>" onclick="if (window.confirm( '<?php echo esc_js( $reset_options_confirm ); ?>' ) )
		...
		public function do_input( $item ) {
			...
			if ( isset( $item['help'] ) ) {
				...
				$help = '<span class="hastip" title="' . __( 'Click to view the screenshot helper', 'tattoopro' ) . '"><a class="wolf-help-img" href="' . esc_url( WOLF_THEME_URI . '/images/admin/help/' . $item['help']  .'.jpg' )  . '"><img src="' . esc_url( WOLF_FRAMEWORK_URI . '/assets/img/help.png' ) . '" alt="help"></a></span>';
		...
		?>
			...
			<tr valign="top" class="<?php echo esc_attr( $class ); ?>"<?php echo $data; ?>>
				...
				<td class="forminp">
					...
					<br>
					<a href="#" class="button wolf-options-reset-img"><?php _e( 'Clear', 'tattoopro' ); ?></a>
					<a href="#" class="button wolf-options-set-img"><?php _e( 'Choose an image', 'tattoopro' ); ?></a>
		...
		<?php elseif ( $type == 'file' ) : ?>
			...
			<tr valign="top" class="<?php echo esc_attr( $class ); ?>"<?php echo $data; ?>>
				...
				<td class="forminp">
					...
					<br><br>
					<a href="#" class="button wolf-options-reset-file"><?php _e( 'Clear', 'tattoopro' ); ?></a>
					<a href="#" class="button wolf-options-set-file"><?php _e( 'Choose a file', 'tattoopro' ); ?></a>
				</td>
			</tr>
		...
		if ( $font_color ) :
			$options = array(
			 ...
			 ?>
			 	<tr valign="top">
			    		<th scope="row" class="titledesc">
						<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_font_color]"><?php _e( 'Font Color', 'tattoopro' ); ?></label>
					</th>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_color]"><?php _e( 'Background Color', 'tattoopro' ); ?><br></label>
				</th>
		...
		?>
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_img]"><?php _e( 'Background Image', 'tattoopro' ); ?>
				</label>
				</th>
				...
				<td class="forminp">
					...
					<br><a href="#" class="button wolf-options-reset-bg"><?php _e( 'Clear', 'tattoopro' ); ?></a>
					<a href="#" class="button wolf-options-set-bg"><?php _e( 'Choose an image', 'tattoopro' ); ?></a>
				</td>
			</tr>
		...
		$options = array( 'no-repeat', 'repeat', 'repeat-x', 'repeat-y' );
		 ?>
		 	<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_repeat]"><?php _e( 'Repeat', 'tattoopro' ); ?></label>
				</th>
		...
		$options = array(
		 ...
		 ?>
	 		<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_position]"><?php _e( 'Position', 'tattoopro' ); ?></label>
				</th>
		...
		if ( $do_attachment ) :
			...
			$options = array(
			 ...
			 ?>
		 		<tr valign="top">
			    		<th scope="row" class="titledesc">
						<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_attachment]"><?php _e( 'Attachment', 'tattoopro' ); ?></label>
					</th>
		...
		---------------*/
		$options = array(
			'cover' => __( 'cover (resize)', 'tattoopro' ),
			'normal' => __( 'normal', 'tattoopro' ),
			'resize' => __( 'responsive (hard resize)', 'tattoopro' ),
		);

		?>
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_size]"><?php _e( 'Size', 'tattoopro' ); ?></label>
				</th>
				...
				<?php if ( $parallax ): ?>
					<tr valign="top">
				    		<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ); ?>[<?php echo esc_attr( $field_id ); ?>_parallax]"><?php _e( 'Parallax', 'tattoopro' ); ?></label>
							</label>
						</th>
		...
		<div class="<?php echo esc_attr( $class ); ?>"<?php echo $data; ?>>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_name]'; ?>"><?php _e( 'Font family', 'tattoopro' ); ?><br>
					</label>
				</th>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_color]'; ?>"><?php _e( 'Font color', 'tattoopro' ); ?><br>
					</label>
				</th>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_weight]'; ?>"><?php _e( 'Font weight', 'tattoopro' ); ?><br>
					<small class="description"><?php _e( 'For example: 400 is normal, 700 is bold.The available font weights depend on the font. Leave empty to use the theme default style', 'tattoopro' ); ?></small>
					</label>
				</th>
			...
			<tr valign="top">
		    		<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_transform]'; ?>"><?php _e( 'Font transform', 'tattoopro' ); ?><br>
					</label>
				</th>
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_style]'; ?>"><?php _e( 'Font style', 'tattoopro' ); ?><br>
					</label>
				</th>
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_font_letter_spacing]'; ?>"><?php _e( 'Letter Spacing (omit px)', 'tattoopro' ); ?><br>
					</label>
				</th>
			...
			<table class="form-table">
				...
				<?php if ( ! in_array( 'type', $exclude_params ) ) : ?>
					<tr valign="top" class="option-section-<?php echo esc_attr( $field_id .'_type' ); ?>">
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_type]'; ?>"><?php _e( 'Video Background type', 'tattoopro' ); ?><br>
						</th>

						<td class="forminp">
							<select name="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_type]'; ?>">
								<option value="youtube" <?php selected( $type, 'youtube' ); ?>>Youtube</option>
								<option value="selfhosted" <?php selected( $type, 'selfhosted' ); ?>><?php _e( 'Self hosted', 'tattoopro' ); ?></option>
							</select>
						</td>
					</tr>
				<?php endif; ?>
				...
				<?php if ( ! in_array( 'youtube_url', $exclude_params ) ) : ?>
					<tr valign="top" class="has-dependency" data-dependency-element="<?php echo esc_attr( $field_id . '_type' ); ?>" data-dependency-values='["youtube"]'>
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_youtube_url]'; ?>"><?php _e( 'Video Background Youtube URL', 'tattoopro' ); ?><br>
						</th>
				...
				<?php if ( ! in_array( 'mp4', $exclude_params ) ) : ?>
					<tr valign="top" class="has-dependency" data-dependency-element="<?php echo esc_attr( $field_id . '_type' ); ?>" data-dependency-values='["selfhosted"]'>
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_mp4]'; ?>"><?php _e( 'mp4 URL', 'tattoopro' ); ?><br>
						</th>
				...
				<?php if ( ! in_array( 'webm', $exclude_params ) ) : ?>
					<tr valign="top" class="has-dependency" data-dependency-element="<?php echo esc_attr( $field_id . '_type' ); ?>" data-dependency-values='["selfhosted"]'>
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_webm]'; ?>"><?php _e( 'webm URL', 'tattoopro' ); ?><br>
						</th>
				...
				<?php if ( ! in_array( 'ogv', $exclude_params ) ) : ?>
					<tr valign="top" class="has-dependency" data-dependency-element="<?php echo esc_attr( $field_id . '_type' ); ?>" data-dependency-values='["selfhosted"]'>
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_ogv]'; ?>"><?php _e( 'ogv URL', 'tattoopro' ); ?><br>
						</th>
				...
				<?php if ( ! in_array( 'image', $exclude_params ) ) : ?>
					<tr valign="top">
						<th scope="row" class="titledesc">
							<label for="<?php echo esc_attr( $prefix ) . '[' .  $field_id .'_img]'; ?>"><?php _e( 'Image URL', 'tattoopro' ); ?><br>
							<small class="description"><?php _e( 'Image fallback', 'tattoopro' ); ?></small></label>
						</th>
