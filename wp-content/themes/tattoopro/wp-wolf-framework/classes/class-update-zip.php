	...
	class Wolf_Update_Zip {
		...
		public function update_zip_form() {
			...
			if ( class_exists( 'ZipArchive' ) ) {
				...
				echo '<br>';
				printf(
					__( '<em>You server configuration does not allow you to update by uploading a zip. You need %s installed on your server. You will have to update the theme via FTP.</em>', 'tattoopro' ),
					'ZipArchive'
				);
			}
		}
		...
		public function form() {
			?>
			<h4><?php _e( 'Update using zip file upload', 'tattoopro' ); ?></h4>
			<p>
				<?php printf(
				__(
					'To update the theme, please upload the %s file from your theme package.', 'tattoopro' ),
			...
			<form action="<?php echo esc_url( admin_url( 'admin.php?page=wolf-theme-update&amp;wolf-theme-just-updated' ) ); ?>" enctype="multipart/form-data" method="post">
				...
				<input type="submit" name="wolf-zip-submit" value="<?php _e( 'Update Theme', 'tattoopro' ); ?>">
			</form>
			<?php
		}
				...
				if ( ! empty( $_FILES['wolf-zip']['name'] ) ) {
					...
					if ( $_FILES['wolf-zip']['name'] == wolf_get_theme_slug() . '.zip' ) {
						if ( $this->unzip( $_FILES['wolf-zip'] ) ) {
							...
							exit();
							//$message = __( 'Theme Updated.', 'tattoopro' );
							//wolf_admin_notice( $message, 'updated' );
							//return false;
						}

					} else {
						$message  = __( 'It seems that you are trying to upload the wrong file.', 'tattoopro' );
						$message .= sprintf( __( 'You have to upload the %s file.', 'tattoopro' ), '<strong>' . wolf_get_theme_slug() . '.zip</strong>' );
		...
		public function unzip( $file ) {
			...
			if ( $res === TRUE ) {
				...
				$message = __( 'An occur occured while trying to update the theme.', 'tattoopro' );
				wolf_admin_notice( $message, 'error' );
				return false;
			}
		}
