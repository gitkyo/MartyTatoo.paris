...
function wolf_theme_css_init() {
	...
	add_settings_field( 'css', __( 'Custom CSS', 'tattoopro' ), 'wolf_theme_css_field', 'wolf-theme-css', 'wolf-theme-css' );
}
...
function wolf_theme_css_intro() {
	...
	<p>
		<?php if ( is_child_theme() ) {
			_e( 'Want to add any custom CSS code? Put in here, and the rest is taken care of.', 'tattoopro' );
		} else {
			 printf(
			 	__( 'Want to add any custom CSS code? Put in here, and the rest is taken care of.<br>If you need more advanced style customization, it is strongly recommended to create a <a href="%s" target="_blank">child theme</a>.', 'tattoopro' ),
...
<div class="wrap">
	...
	<h2><?php _e( 'Custom CSS', 'tattoopro' ); ?></h2>
	...
	<p>
		<form action="<?php echo esc_url( admin_url( 'admin.php?page=wolf-theme-css' ) ); ?>" method="post">
			...
			<input id="wolf-theme-css-submit" name="wolf-theme-css-save" type="submit" class="button-primary" value="<?php _e( 'Save Changes', 'tattoopro' ); ?>" />
		</form>
	</p>
	<div class="clear"></div>
</div>