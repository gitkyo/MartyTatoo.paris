...
if ( ! function_exists( 'wolf_change_comment_form_defaults' ) ) {
	...
	function wolf_change_comment_form_defaults( $default ) {
		...
		<label for="email">' . __( 'Email', 'tattoopro' ) . ' ' . ( $req ? '<span class="required">*</span>' : '' ) . '</label><input id="email" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';
		return $default;
	}
