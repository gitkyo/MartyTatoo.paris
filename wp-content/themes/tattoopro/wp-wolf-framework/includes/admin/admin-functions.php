...
if ( ! function_exists( 'wolf_admin_notice' ) ) {
	...
	function wolf_admin_notice( $message = null, $type = null, $dismiss = false, $id = null ) {
		...
		if ( $dismiss ) {
			...
			$dismiss = __( 'Hide permanently', 'tattoopro' );
	...
	function wolf_theme_update_notification_message( $link = true ) {
		...
		if ( WOLF_UPDATE_NOTICE ) {
			...
			if ( $changelog && isset( $changelog->latest ) && -1 == version_compare( WOLF_THEME_VERSION, $changelog->latest ) ) {
				$message  = '';
				$message .= '<strong>' . sprintf( __( 'There is a new version of %s available.', 'tattoopro' ),  ucfirst( wolf_get_theme_slug() ) ) . '</strong>';
				$message .= sprintf( __( 'You have version %s installed.', 'tattoopro' ),  WOLF_THEME_VERSION );
				if ( $link ) {
					$message .= '<a href="' . esc_url( admin_url( 'admin.php?page=wolf-theme-update' ) ) . '">';
				}
					$message .= ' ' . sprintf( __( 'Update to version %s', 'tattoopro' ),  $changelog->latest );
	...
	function wolf_check_folder( $dir ) {
		...
		elseif ( ! is_dir( $dir ) ) {
			...
			if ( ! mkdir( $dir, 775 ) ) {
				$message  = sprintf(
					__(
						'Error while trying to find the folder <strong>%1$s</strong>. Please create it manually and set the permission to 775 through your FTP client<br>or with the command line <strong>mkdir -m 775 %2$s</strong>', 'tattoopro'
					), $dir, $dir
				);
