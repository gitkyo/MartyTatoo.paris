...
if ( ! class_exists( 'Wolf_Framework_Admin' ) ) {
	...
	class Wolf_Framework_Admin {
		...
		public function menu() {
			...
			add_submenu_page( 'wolf-theme-options', __( 'Options', 'tattoopro' ), __( 'Options', 'tattoopro' ), 'manage_options', 'wolf-theme-options', array( $this, 'options' ) );
			add_submenu_page( 'wolf-theme-options', __( 'Custom CSS', 'tattoopro' ), __( 'Custom CSS', 'tattoopro' ), 'manage_options', 'wolf-theme-css', array( $this, 'css' ) );

			/* If update notice is enabled, we add a theme update page */
			if ( WOLF_UPDATE_NOTICE ) {

				$menu_title = __( 'Updates', 'tattoopro' );
				if ( $xml = wolf_get_theme_changelog() ) {
					if ( version_compare( WOLF_THEME_VERSION, $xml->latest ) == -1 ) {
						$menu_title = __( 'Updates', 'tattoopro' ) . '<span class="update-plugins count-1 wolf-custom-count"><span class="update-count">1</span></span>';
					}
				}

				add_submenu_page( 'wolf-theme-options', __( 'Updates', 'tattoopro' ), $menu_title, 'manage_options', 'wolf-theme-update', array( $this, 'update_page' ) );
			}

				// Support forum link/page
			if ( WOLF_SUPPORT_PAGE ) {

				add_submenu_page( 'wolf-theme-options', __( 'Helpdesk', 'tattoopro' ), __( 'Helpdesk', 'tattoopro' ), 'manage_options', 'wolf-theme-support', array( $this, 'support_page' ) );
			}
		...
		public function display_notice() {
			...
			if ( WOLF_ERROR_NOTICES ) {
				...
				// No cURL
				$no_cURL = __( 'The <strong>cURL</strong> extension is not installed on your server. This extension is required to display theme update notifications.', 'tattoopro' );
				...
				// No GD library
				$no_GD_library = __( 'The <strong>GD library</strong> extension is not installed on your server. This extension is essential to Wordpress to be able to resize your images. Please contact your hosting service for more informations.', 'tattoopro' );
			...
			$wrong_install = sprintf(
				__( 'It seems that <strong>the theme has been installed incorrectly</strong>. Go <a href="%s" target="_blank">here</a> to find instructions about theme installation.', 'tattoopro' ), 'http://wolfthemes.com/common-wordpress-theme-issues/'
				);
