...
<article id="post-<?php the_ID(); ?>" <?php post_class( array( 'gallery-item-container', $term_list ) ); ?>>
	<figure class="effect-bubba">
		...
		<a href="<?php the_permalink(); ?>" class="mask-link"><?php _e( 'View gallery', 'tattoopro' ); ?></a>
		<figcaption>
			<div class="figcaption-inner table">
				<div class="table-cell" >
					<h2 class="gallery-title"><?php the_title(); ?></h2>
					<p class="entry-meta">
						<?php echo sanitize_text_field( strip_tags( get_the_term_list( $post_id, 'gallery_type', __( 'in ', 'tattoopro' ), ', ', '' ) ) ); ?>
					</p>
				</div>
			</div>
		</figcaption>
